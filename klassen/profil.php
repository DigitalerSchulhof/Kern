...
class Profil {
  ...
  public function getProfil() : UI\Reiter {
    ...
    if ($nutzerkonto !== null) {
      ...
      $benutzerF = (new UI\Textfeld("dshProfil{$profilid}Benutzer"))->setWert($this->person->getBenutzer());
      ...
      if ($DSH_BENUTZER->hatRecht("$recht.einstellungen")) {
        ...
        if ($DSH_BENUTZER->getId() == $this->person->getId()) {
          ...
          $formular         = new UI\FormularTabelle();
          $formular[]       = new UI\FormularFeld(new UI\InhaltElement("Aktiv:"),          (new UI\IconToggle("dshProfil{$profilid}EmailAktiv", "eMails über den Schulhof verwalten", new UI\Icon(UI\Konstanten::HAKEN)))->setWert($mailaktiv));
          $formular[]       = new UI\FormularFeld(new UI\InhaltElement("eMail-Adresse:"),             (new UI\Mailfeld("dshProfil{$profilid}EmailAdresse"))->setWert($mailadresse));
          $formular[]       = new UI\FormularFeld(new UI\InhaltElement("Anzeigename:"),               (new UI\Textfeld("dshProfil{$profilid}EmailName"))->setWert($mailname));
          $formular[]       = new UI\FormularFeld(new UI\InhaltElement("IMAP-Host (Posteingang):"),   (new UI\Textfeld("dshProfil{$profilid}EmailEingangHost"))->setWert($mailehost));
          $formular[]       = new UI\FormularFeld(new UI\InhaltElement("IMAP-Port (Posteingang):"),   (new UI\Zahlenfeld("dshProfil{$profilid}EmailEingangPort",0,65535))->setWert($maileport));
          $formular[]       = new UI\FormularFeld(new UI\InhaltElement("Benutzername (Posteingang):"),(new UI\Textfeld("dshProfil{$profilid}EmailEingangNutzer"))->setWert($mailenutzer));
          $formular[]       = (new UI\FormularFeld(new UI\InhaltElement("Passwort (Posteingang):"),    (new UI\Passwortfeld("dshProfil{$profilid}EmailEingangPasswort"))->setWert($mailepasswort)))->setOptional(true);
          $formular[]       = new UI\FormularFeld(new UI\InhaltElement("SMTP-Host (Postausgang):"),   (new UI\Textfeld("dshProfil{$profilid}EmailAusgangHost"))->setWert($mailahost));
          $formular[]       = new UI\FormularFeld(new UI\InhaltElement("SMTP-Port (Postausgang):"),   (new UI\Zahlenfeld("dshProfil{$profilid}EmailAusgangPort",0,65535))->setWert($mailaport));
          $formular[]       = (new UI\FormularFeld(new UI\InhaltElement("Benutzername (Postausgang):"),(new UI\Textfeld("dshProfil{$profilid}EmailAusgangNutzer"))->setWert($mailanutzer)))->setOptional(true);
          $formular[]       = (new UI\FormularFeld(new UI\InhaltElement("Passwort (Postausgang):"),    (new UI\Passwortfeld("dshProfil{$profilid}EmailAusgangPasswort"))->setWert($mailapasswort)))->setOptional(true);
